...
function recomputeApertureStateFromOptionsAndScrollTop({ preloadBatchSize, preloadAdditionalHeight, infiniteComputer }, scrollTop) {
    var blockNumber = preloadBatchSize === 0 ? 0 : Math.floor(scrollTop / preloadBatchSize), blockStart = preloadBatchSize * blockNumber, blockEnd = blockStart + preloadBatchSize, apertureTop = Math.max(0, blockStart - preloadAdditionalHeight), apertureBottom = Math.min(infiniteComputer.getTotalScrollableHeight(), blockEnd + preloadAdditionalHeight);
    return {
...
function generateComputedProps(props) {
    ...
    let me = this;
    var { containerHeight, preloadBatchSize, preloadAdditionalHeight, handleScroll, onInfiniteLoad, ...oldProps } = props;
    var newProps = {};
    ...
    else if (typeof batchSize === 'object' &&
